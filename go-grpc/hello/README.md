...
- For No-TLS and No-Reflection gRPC service
    ...
    - Reflection is another gRPC feature to expose the gRPC schema to consumers.
      ...
      the API schema. Otherwise, we need to supply either:
        - -protoset <a compiled binary from .proto> (or)
    ...
    - Examples using plain proto
        ...
        # Note: Must use -import-path <path> option in case your .proto imports other proto files in the path.
        ```
    - Examples using compiled protoset
        ```
        # Compile the proto using (also required for every change in .proto file)
        protoc --descriptor_set_out=proto/hello.protoset proto/hello.proto
        # (or) Compile with include path if you have dependent proto files
        protoc --proto_path=. --include_imports \
               --descriptor_set_out=proto/hello.protoset proto/hello.proto

        # RPC call using compiled protoset
...
- For gRPC services that support TLS and reflection
    ```
